...
exports.getUsersSubscriptionToAdmin = async (req, res, next) => {
    try {
        ...
        const subscription = await subscriptionModel.find({ userId: id }).select('-_id -userId -adminId -__v');
...
exports.getUsersSubscriptionToUser = async (req, res, next) => {
    try {
        ...
        const subscription = await subscriptionModel.find({ userId: user_id }).select('-_id -userId -adminId -__v');
        if (!subscription) {
            return res.status(400).json({ message: "User has no subscription" });
        }
        return res.status(200)
...
exports.updateSubscription = async (req, res, next) => {
    try {
        ...
        const parsedDueDate = new Date(duedate);
        const updatedSubscription = { subscription, amount, gst, duedate: parsedDueDate, userId: id, adminId: admin_id }
        if (!id) {
        ...
        const subscriptionResponse = await subscriptionModel.findByIdAndUpdate(subscriptionId, updatedSubscription, { new: true });
